...
function AdminDashboard() {
  ...
  const removeRecipient = (index) => {
    if (broadcastForm.recipients.length > 1) {
      const newRecipients = broadcastForm.recipients.filter(
        (_, i) => i !== index
      );
      setBroadcastForm((prev) => ({
        ...prev,
  ...
  const handleBroadcastSubmit = async (e) => {
    ...
    // Validation
    const validRecipients = broadcastForm.recipients.filter(
      (email) => email.trim() !== ""
    );

    if (validRecipients.length === 0) {
      showAlert("Please add at least one recipient", "error");
      return;
    }
                    ...
                    <div className="recipients-section">
                      ...
                      >
                        Select All Active Users (
                        {
                          domainUsers.filter(
                            (u) =>
                              u.status === "active" && u.email !== adminEmail
                          ).length
                        }
                        )
                      </button>
                    ...
                    <div className="success-result">
                      ...
                      <div className="result-stats">
                        <p>
                          ...
                          {broadcastResult.data.total_requested}
                        </p>
                        <p>
                          <strong>Valid Recipients:</strong>{" "}
                          {broadcastResult.data.total_valid}
                        </p>
                        <p>
                          <strong>Successfully Sent:</strong>{" "}
                          {broadcastResult.data.total_sent}
                        </p>
                        <p>
                          <strong>Failed:</strong>{" "}
                          {broadcastResult.data.total_failed}
                        </p>
                        <p>
                          <strong>Success Rate:</strong>{" "}
                          {broadcastResult.data.summary?.success_rate}
                        </p>
                        <p>
                          <strong>Processing Time:</strong>{" "}
                          {broadcastResult.data.summary?.processing_time}
                        </p>
                      </div>
                      ...
                      {broadcastResult.data.invalid_emails?.length > 0 && (
                        <div className="invalid-emails">
                          ...
                          <ul>
                            {broadcastResult.data.invalid_emails.map(
                              (invalid, index) => (
                                <li key={index}>
